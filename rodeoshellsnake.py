...
def event_loop():
    ...
    global again, in_action, in_action_bis, nu_head_position, program,in_action1,in_action_bis1,nu_head_position1
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == K_UP and in_action != "K_DOWN":
                ...
                in_action_bis = True
                nu_head_position[0] -= 20
            if event.key == K_z and in_action1 != "K_s":
                in_action1 = "K_z"
                in_action_bis1 = True
                nu_head_position1[1] -= 20

            elif event.key == K_s and in_action1 != "K_z":
                in_action1 = "K_s"
                in_action_bis1 = True
                nu_head_position1[1] += 20

            elif event.key == K_d and in_action1 != "K_q":
                in_action1 = "K_d"
                in_action_bis1 = True
                nu_head_position1[0] += 20

            elif event.key == K_q and in_action1 != "K_q":
                in_action1 = "K_LEFT"
                in_action_bis1 = True
                nu_head_position1[0] -= 20
    ...
    in_action_bis = False
    in_action_bis1= False

#########################################################################################
def update():
    """Fonction mettant à jour les données contenu
    de l'évènement passé."""
    global food_position, old_food_position, score, snake_positions,snake_positions1
    ...
    if nu_head_position1 != food_position:
        # Insertion d'une nouvelle tête:
        snake_positions1.pop(-1)

        snake_positions1.insert(0, list(nu_head_position1))

    # Quand le serpent mange la nourriture:
    elif nu_head_position== food_position or nu_head_position1==food_position:
        # Insertion d'une nouvelle tête à l'emplacement de la nourriture:
        snake_positions.insert(0, food_position)
        snake_positions1.insert(0, food_position)
        ...
        while True:
            ...
            if food_position not in (snake_positions or snake_positions1):
                break

    # Si le serpent sort de la surface de jeu ou si il se mord la queue:
    if snake_positions[0][0] < 0 or snake_positions[0][0] > 480 or snake_positions[0][1] < 100 or snake_positions[0][1] > 480 or snake_positions[0] in snake_positions[1:]or snake_positions1[0][0] < 0 or snake_positions1[0][0] > 480 or snake_positions1[0][1] < 100 or snake_positions1[0][1] > 480 or snake_positions1[0] in snake_positions1[1:]:
...
def draw():
    ...
    global food, score_surface, snake_positions,snake_positions1
    ...
    # Si le serpent mange:
    if nu_head_position == old_food_position or nu_head_position1== old_food_position:
        ...
        for index1, item1 in enumerate(snake_positions1):
            # On dessine la tête en orange:
            if index1 == 0:
                pygame.draw.rect(screen, (ORANGE), Rect(item1[0], item1[1], 20, 20))
            # On dessine le reste du corps en vert:
            else:
                pygame.draw.rect(screen, (VERT), Rect(item1[0], item1[1], 20, 20))
        ...
        for index1, item1 in enumerate(snake_positions1):
            # On dessine la tête en orange (contours uniquement):
            if index1 == 0:
                pygame.draw.rect(screen, (ORANGE), Rect(item1[0], item1[1], 20, 20), 2)
            # On dessine le reste du corps en vert (contours uniquement):
            else:
                pygame.draw.rect(screen, (VERT), Rect(item1[0], item1[1], 20, 20), 2)
...
while again:
    ...
    snake1 = pygame.draw.rect(screen, (ORANGE), Rect(0, 240, 20, 20), 2)
    snake_positions = [[0, 240]]
    snake_positions1 = [[0, 240]]
    #
